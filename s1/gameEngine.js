...
// Author: David Cline 
// Minor edits by: Michael Farcasin
//----------------------------------------------------------------------//
...
var debug = function(message, textIterable)
{
	//console.log(message);
	if (engine.DEBUG && engine.canvas !== undefined)
	{
		document.getElementById("debugContainer").style.left  = (engine.canvas.offsetLeft + 5) + 'px';
		...
		{
			var fromIndex = n-500;
			var startPoint = engine.debugText.indexOf('\n', fromIndex)+1;
			engine.debugText = engine.debugText.substring(startPoint);
		}
	...
	if ("children" in jsonNode)
	{
		...
		for (var i=0; i<children.length; i++)
		{
			...
			if (childType == "perspectiveCamera") {
				...
				var plane = parseGround(childJsonNode);
				sceneNode.add(plane);
...
function parseHemisphereLight(jsonNode)
{
	...
	// Replace with data from jsonNode
	if ("skyColor" in jsonNode) skyColorData = jsonNode["skyColor"];
...
//----------------------------------------------------------------------//
// PARSE A GROUND PLANE
//----------------------------------------------------------------------//

function parseGround(jsonNode)
{
	//debug("parseMesh\n");
	...
	if (type == "meshBasicMaterial")
	{
		...
		if ("side" in jsonNode) {
			...
			switch(jsonNode["side"]) {
				case "back": s = THREE.BackSide; break;
				case "both": s = THREE.DoubleSide; break;
				default: s = THREE.FrontSide; break;
			}
			material.side = s;
		}
		return material;
...
function translateCamera(vector, changeCenter)
{
	if (changeCenter === undefined) changeCenter = true;
	var centerVector = new THREE.Vector3( 0, 0, -1 );
	centerVector.applyQuaternion( gameState.camera.quaternion );
	
	var eye = gameState.camera.position;
	gameState.camera.position.set(eye.x + vector[0], eye.y + vector[1], eye.z + vector[2]);
	
	if (!changeCenter) {
		gameState.camera.lookAt( centerVector );
	}
	
	//debug('eye: ['+gameState.camera.position.x+','
