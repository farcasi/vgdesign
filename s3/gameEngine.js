...
var engine = {
	GAME_CONTAINER_ID: 'centerFrame',
	CANVAS_ID: 'gameCanvas',
	DEBUG_CONTAINER_ID: 'debugContainer',
...
function parseSceneNode(jsonNode, sceneNode)
{
	...
	if ("children" in jsonNode)
	{
		...
		for (var i=0; i<children.length; i++)
		{
			...
			if (typeof(childType) == "string") childType = childType.toLowerCase();
			...
			else if (childType.toLowerCase().includes("camera")) {
...
function getForwardDirection()
{
	...
	var threeVector = camVector.unproject(gameState.camera);
	return threeVector.sub(gameState.camera.position).normalize();
}
